...
# Executed as a standalone Python script in a tmux session for long-running training (random seed = 77)
...
seed = 77  # setting random seed for reproducible training
...
PROJECT_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..")) # defining project paths relative to repository root
...
print("\n Loading pretrained ResNet-50 model...")
model = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V2) # loading ImageNet-pretrained ResNet-50 backbone
...
np.save(history_name, history)  #  Early stopping 
